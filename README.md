# QCC Enterprise Query

基于企查查公开接口的企业查询 skill，当前封装了 3 个接口：

- `1. 企业工商信息`：查基础工商主体信息，成本更省
- `2. 企业信息核验`：查增强核验字段，信息更完整，但费用更高
- `3. 企业模糊搜索`：按简称、电话、地址、人名、产品名、经营范围等线索找企业

## 快速用法

### 1. 首次使用先配置凭证

```bash
python3 scripts/manage_credentials.py --app-key "你的QCC_KEY" --secret-key "你的QCC_SECRET_KEY"
```

查看当前凭证状态：

```bash
python3 scripts/manage_credentials.py
```

凭证会写入 `qcc-enterprise-query/.env`，后续复用。

### 2. 常见查询方式

自动路由查询：

```bash
python3 scripts/run_query.py --company-name "企查查科技股份有限公司" --request "查企业信息"
```

查第 `1` 种基础工商信息：

```bash
python3 scripts/registration_details.py --company-name "企查查科技股份有限公司"
```

查第 `2` 种增强核验信息：

```bash
python3 scripts/basic_details.py --company-name "企查查科技股份有限公司"
```

按线索模糊搜索：

```bash
python3 scripts/fuzzy_search.py --search-key "010-62621818"
```

## 路由规则

- 只给企业全称或统一社会信用代码时：
  先让用户选择 `1` 或 `2`
- 只给简称、电话、地址等线索时：
  先走企业模糊搜索，只确认企业全称；确认后再单独说明 `1/2` 区别并让用户选择
- 只要即将执行第 `2` 种查询：
  必须先提示费用更高，并让用户二次确认
- 如果用户直接问 `人员规模 / 参保人数 / 联系方式 / 更多邮箱 / 国标行业 / 企业性质`：
  仍先说明 `1/2` 的区别并推荐第 `2` 种，用户选中后再进入费用确认

## 第 2 种查询的二次确认

如果已经确认要走第 `2` 种查询，需要再次确认后才会正式查询：

```bash
python3 scripts/run_query.py \
  --company-name "企查查科技股份有限公司" \
  --request "确认" \
  --detail-api 2001 \
  --confirm-expensive \
  --original-request "查人员规模和参保人数"
```

确认提示建议使用：

- `你选的是企业信息核验。`
- `这个查询费用较高。`
- `如果确认继续，请直接回复 确认。`
- `如果无需查询参保人数、邮箱电话等信息，可以选择基础工商信息，回复 企业工商信息 或 1即可。`

## 适合怎么问

- `查一下“杭州飞致云信息科技有限公司”的企业信息`
- `查一下“杭州飞致云信息科技有限公司”的人员规模和参保人数`
- `通过电话“010-62621818”找企业`

## 交互顺序

1. 企业名称不完整时，先走 `886` 返回候选企业，只让用户确认企业全称。
   候选列表要带序号，用户可以直接回复企业全称，或者回复 `第几个/序号`。
2. 企业全称确认后，再单独介绍：
   `1. 企业工商信息` 通常包含企业名称、统一社会信用代码、法定代表人、登记状态、成立日期、注册资本、企业类型、注册地址、经营范围、营业期限、登记机关、核准日期、上市状态等。
   `2. 企业信息核验` 在此基础上额外补充企业性质、人员规模、参保人数、国标行业、企查查行业、电话、更多电话、邮箱、更多邮箱、网址、曾用名等增强字段。
3. 用户选择第 `2` 种后，再做一次费用确认，确认后才正式查询。
4. 查询完成后的后续建议，只能继续推荐本 skill 已支持的内容。
   不要提及股东信息、对外投资、司法风险、历史变更等未封装能力。

## 相关文件

- [SKILL.md](./SKILL.md)：skill 规则与对话约定
- [references/api-routing.md](./references/api-routing.md)：接口路由说明
- [references/examples.md](./references/examples.md)：示例输入与期望行为
