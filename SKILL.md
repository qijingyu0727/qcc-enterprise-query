...
7. 用户确认完整企业全称后，再说明 `410` 和 `2001` 的区别，并让用户选择其一。
8. 如果用户一开始就给了完整企业全称或统一社会信用代码，也不要直接查详情；先让用户选择 `410` 或 `2001`。
9. 只要即将调用 `2001`，都必须先提示这类查询费用更高，并做一次二次确认；用户确认后，才调用 `scripts/basic_details.py`。
10. 如果用户直接问 `人员规模 / 参保人数 / 联系方式 / 更多邮箱 / 国标行业 / 企业性质` 等增强字段，优先进入 `2001` 的费用确认，不必再重复做普通接口选择。
11. 用户已明确说“用 410”时，可直接调用 `scripts/registration_details.py`；用户已明确说“用 2001”时，也先走费用确认，再调用 `scripts/basic_details.py`。
12. 默认推荐 `410`；如果诉求涉及 `人员规模 / 参保人数 / 联系方式 / 更多邮箱 / 国标行业 / 企业性质` 等增强字段，明确推荐 `2001`。
...
- 用户直接要求增强字段，或已经选择/明确指定 `2001`：
  先返回“费用较高，是否确认查询”的确认提示。
- 用户已明确选 `410`：
