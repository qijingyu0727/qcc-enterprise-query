...
9. 企业全称确认完成后，再单独说明 `410` 和 `2001` 的区别，不要把“企业确认”和“查询类型确认”放到同一轮里。
10. 说明查询类型时，要明确讲清：
   `410 企业工商信息` 通常包含哪些基础工商字段；
   `2001 企业信息核验` 在此基础上额外多出哪些增强字段。
11. 只要即将调用 `2001`，都必须先提示这类查询费用更高，并做一次二次确认；用户确认后，才调用 `scripts/basic_details.py`。
12. 用户已明确说“用 410”时，可直接调用 `scripts/registration_details.py`；用户已明确说“用 2001”时，也先走费用确认，再调用 `scripts/basic_details.py`。
13. 默认推荐 `410`；如果诉求涉及 `人员规模 / 参保人数 / 联系方式 / 更多邮箱 / 国标行业 / 企业性质` 等增强字段，明确推荐 `2001`，但仍要先做 `410/2001` 选择说明。
