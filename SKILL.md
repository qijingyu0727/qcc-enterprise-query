...
11. 如果用户直接要求 `人员规模 / 参保人数 / 联系方式 / 更多邮箱 / 国标行业 / 企业性质` 等增强字段，则进入企业信息核验链路。
...
16. 如果用户先给的是简称、电话、地址、人名、产品名、经营范围等线索，先做模糊搜索；一旦用户确认了企业，而原始诉求本来就是增强字段或企业信息核验，则先提示这类查询费用较高，用户回复 `确认` 后再继续查企业信息核验。
17. 所有用户可见输出里，不要出现 `410 / 2001 / 886 / ApiCode / 脚本名` 等内部技术信息。
...
- 用户是通过简称、电话、地址、人名、产品名、经营范围等线索先找到候选企业，且原始诉求已明确是增强字段：
  企业确认后直接执行企业信息核验。
- 用户在基础结果后回复 `需要`：
  直接执行第 `2` 种查询，不再额外确认。
- 用户已经选择/明确指定第 `2` 种：
...
- 用户可见输出不要展示 `410 / 2001 / 886 / ApiCode / registration_details.py / basic_details.py / fuzzy_search.py`。
- 基础结果返回后，要明确告诉用户：
