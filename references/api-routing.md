...
6. 用户一开始就给了完整企业全称或统一社会信用代码：
   也先让用户选择 `410` 或 `2001`
7. 用户直接要求增强字段，但尚未明确选 `2001`：
   仍先返回 `detail_api_selection`，说明 `410/2001` 的区别并推荐 `2001`
8. 用户已经选择/明确指定 `2001`：
   再返回 `expensive_confirmation`，提示该查询费用更高并等待确认
9. 用户明确选 `410`：
   调 `registration_details.py`
10. 用户确认继续执行高价 `2001` 查询：
   调 `basic_details.py`
11. 详情接口未命中：
   自动回退 `886` 重新找候选企业
...
- 需要用户确认高价 `2001` 时，返回 `expensive_confirmation`
- 候选企业确认阶段，返回 `clarification`
- `410` 默认适合基础工商信息
- `2001` 默认适合增强核验字段
- `detail_api_selection` 需要明确区分：
  `410` 包含哪些基础工商字段；
  `2001` 比 `410` 额外多出哪些增强字段
- 用户请求增强字段但选择 `410` 时，要明确提示可改用 `2001`
